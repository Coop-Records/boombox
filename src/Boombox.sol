...
contract Boombox {
    ...
    function setDefaultTierPercentages(
        uint[] memory defaults
    ) external onlyAdmin {
        for (uint i = 0; i <= uint(Tier.Diamond); i++) {
    ...
    function distribute(string memory artistId) external {
        ...
        for (uint t = uint(Tier.Diamond) + 1; t > 0; t--) {
            ...
            uint pointsForTier = (pointsToDistribute[artistId] *
                tierPercentage) / totalPercentage;
            ...
            for (uint j = 0; j < participants[artistId].length; j++) {
                ...
                if (participantTier[artistId][participant] >= Tier(i)) {
                    uint pointsPerParticipant = pointsForTier /
                        participantsInTier;
