...
interface IPoints {
    ...
    event PointsAdded(
        ...
        uint256 points
    ); // Emitted when points are added to a user's allocation.
    event PointsSubtracted(
        uint256 indexed systemId,
        address indexed user,
        uint256 points
    ); // Emitted when points are subtracted from a user's allocation.
    ...
    event DropCreated(uint256 indexed dropId, uint256 indexed systemId); // Emitted when a new drop is created.
    event PointsRedeemable(
        uint256 indexed systemId,
        address indexed user,
        uint256 points
    ); // Emitted when points are made redeemable for a user.
     ...
     * @param points The amounts of points to add.
     */
    function addPointsBatchMultipleSystems(
        uint256[] calldata systemIds,
        address[] calldata users,
        uint256[] calldata points
    ) external;
     ...
     * @param points The amounts of points to subtract.
     */
    function subtractPointsBatchMultipleSystems(
        uint256[] calldata systemIds,
        address[] calldata users,
        uint256[] calldata points
    ) external;
